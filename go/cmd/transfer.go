...
// applyFlagStringBothSides mirrors applyFlagIntBothSides for a single CLI
// flag that sets both the extract-phase and migrate-phase value at once
// (#583) — the config file's source/target sections are the only way to
// give the two phases different --branch_analyzed_after values on a
// transfer run, exactly like --concurrency / --timeout.
