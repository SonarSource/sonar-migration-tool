...
var commandFlags = map[string][]string{
	...
	"extract": {
		"--branch_analyzed_after", "--branch_regexp", "--cert_password",
	...
	"migrate": {
		"--api_max_rate_per_min", "--branch_analyzed_after", "--branch_regexp",
...
func TestTier0_ExitCodeMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:         "extract_with_unparseable_branch_regexp",
			args:         []string{"extract", "--config", cfg, "--export_directory", exportDir, "--branch_regexp", "("},
			wantExit:     1,
			wantContains: `invalid branch regexp pattern "("`,
		},
		{
			name:         "migrate_with_unparseable_branch_regexp",
			args:         []string{"migrate", "--config", cfg, "--export_directory", exportDir, "--branch_regexp", "("},
			wantExit:     1,
			wantContains: `invalid branch regexp pattern "("`,
		},
		{
			name:         "transfer_with_unparseable_branch_regexp",
			args:         []string{"transfer", "--config", cfg, "--project_key", "x", "--branch_regexp", "("},
			wantExit:     1,
			wantContains: `invalid branch regexp pattern "("`,
		},
		{
