...
var commandFlags = map[string][]string{
	...
	"extract": {
		"--branch_analyzed_after", "--cert_password", "--concurrency", "--config",
		"--export_directory", "--extract_id", "--extract_type",
		"--history_max_points", "--history_min_interval_days", "--insecure",
		"--key_file_path", "--migrate_history", "--objects", "--pem_file_path",
		"--project_key", "--skip_issue_sync", "--skip_project_data_migration",
		"--source_token", "--source_url", "--target_task", "--timeout",
	},
	"gui":      {"--addr", "--config", "--export_directory", "--no-browser"},
	"mappings": {"--config", "--export_directory"},
	"migrate": {
		"--branch_analyzed_after", "--concurrency", "--config",
		"--default_organization", "--edition", "--enterprise_key",
		"--exclude_branches", "--export_directory", "--fast_sync",
		"--migrate_history", "--objects", "--project_data_build_concurrency",
		"--project_key", "--project_key_pattern", "--run_id",
		"--skip_issue_sync", "--skip_profiles", "--skip_project_data_migration",
		"--target_task", "--target_token", "--target_url", "--timeout",
	},
	...
	"transfer": {
		"--branch_analyzed_after", "--cert_password", "--concurrency", "--config",
		"--default_organization", "--edition", "--enterprise_key",
		"--exclude_branches", "--export_dir", "--fast_sync",
		"--history_max_points", "--history_min_interval_days", "--insecure",
		"--key_file_path", "--migrate_history", "--pem_file_path", "--project_key",
		"--project_key_pattern", "--skip_issue_sync",
...
func TestTier0_ExitCodeMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:         "extract_with_bad_branch_analyzed_after",
			args:         []string{"extract", "--config", cfg, "--export_directory", exportDir, "--branch_analyzed_after", "not-a-date"},
			wantExit:     1,
			wantContains: `invalid branch_analyzed_after value "not-a-date": expected YYYY-MM-DD format`,
		},
		{
			name:         "migrate_with_bad_branch_analyzed_after",
			args:         []string{"migrate", "--config", cfg, "--export_directory", exportDir, "--branch_analyzed_after", "2026-13-40"},
			wantExit:     1,
			wantContains: `invalid branch_analyzed_after value "2026-13-40": expected YYYY-MM-DD format`,
		},
		{
			name:         "transfer_with_bad_branch_analyzed_after",
			args:         []string{"transfer", "--config", cfg, "--project_key", "x", "--branch_analyzed_after", "bogus"},
			wantExit:     1,
			wantContains: `invalid branch_analyzed_after value "bogus": expected YYYY-MM-DD format`,
		},
		{
