...
var commandFlags = map[string][]string{
	...
	"extract": {
		"--branch_regexp", "--cert_password", "--concurrency", "--config",
		"--export_directory", "--extract_id", "--extract_type",
		"--history_max_points", "--history_min_interval_days", "--key_file_path",
		"--migrate_history", "--objects", "--pem_file_path", "--project_key",
		"--skip_issue_sync", "--skip_project_data_migration", "--source_token",
		"--source_url", "--target_task", "--timeout",
	},
	"gui":      {"--addr", "--config", "--export_directory", "--no-browser"},
	"mappings": {"--config", "--export_directory"},
	"migrate": {
		"--branch_regexp", "--concurrency", "--config", "--default_organization",
		"--edition", "--enterprise_key", "--exclude_branches", "--export_directory",
	...
	"transfer": {
		...
		"--history_max_points", "--history_min_interval_days",
