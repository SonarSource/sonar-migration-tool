...
- Added **`--max_issue_comments` option** to `migrate` and `transfer` — caps the number of most-recent source comments replayed onto each migrated issue/hotspot (default `5`, max `20`), reducing SonarQube Cloud API pressure on long comment threads. Overridable via `target.max_issue_comments` in the config file. ([#571](https://github.com/SonarSource/sonar-migration-tool/issues/571))
