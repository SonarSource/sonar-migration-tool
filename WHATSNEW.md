...
- **`--fast_sync` option** — skips issue/hotspot sync work for findings that are still in their original, untouched state (`OPEN` issues, `TO_REVIEW` hotspots with no tags or comments), significantly speeding up large migrations. ([#527](https://github.com/SonarSource/sonar-migration-tool/issues/527))
