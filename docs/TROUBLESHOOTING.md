...
A branch recorded as `up_to_date` counts as succeeded and is skipped too: the target already holds an analysis at or after the date this run would submit, so there is nothing left to import. A branch dropped by the per-project branch limit is the exception — it is re-evaluated on every run, so raising `--max_branches_per_project` (or relaxing `--exclude_branches`, `--branch_regexp` or `--branch_analyzed_after`) and resuming will migrate it.
