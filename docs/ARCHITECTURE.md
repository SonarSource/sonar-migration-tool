...
- **Project Data** — Import scan reports via reconstructed protobuf format (native issues, external issues via ExternalIssue protobuf, and Security Hotspots converted to native issues by `scanreport.ConvertHotspotsToIssues`). **Hotspots have no separate representation in the scanner protocol:** the native `Issue` protobuf message has no type field, so a report can only name a rule and let the server classify the finding — which is why hotspots have always arrived on the target as ordinary issues rather than being lost. The conversion therefore emits rule coordinates, message and text range only, with no type, no impacts and **no severity override** (the previous `vulnerabilityProbability` → CRITICAL/MAJOR/MINOR squash stamped a severity the rule would never raise; SonarQube Cloud derives type and severity from the rule, which since 2026-07-01 is an ordinary issue rule carrying SonarSource's `former-hotspot` system tag). Converted hotspots then pass through the **same** `dropIssuesWithInactiveRules` filter as native issues — they used to bypass it, which is unsafe now that a hotspot rule can be `REMOVED` on Cloud and an issue naming a rule the analysis does not activate aborts the *entire* report in the CE; dropped ones are counted and logged. See [HOTSPOTS-AS-ISSUES.md](HOTSPOTS-AS-ISSUES.md). BackdateChangesets mechanism preserves original issue creation dates; external issues are included in changeset backdating alongside native issues. Project version (`sonar.projectVersion`) is migrated from SonarQube Server to SonarQube Cloud: the extracted version is set in both the protobuf metadata and the CE submit form, falling back to `"1.0.0"` if unavailable (matching CloudVoyager behavior). Harvested from CloudVoyager's `resolve-source-project-version.js`. **Multi-branch handling:** Branches are sorted main-first via `sortBranchesMainFirst()`. The main branch is imported first and its CE task awaited; the non-main branches of a project then run **concurrently with one another** (#596), each migrated as a **long-lived branch with full issue history** — `buildBranchReport` first performs SonarQube Cloud's "Create analysis" handshake (`PreCreateAnalysis` → `POST {api-host}/analysis/analyses` with `branchType=long`) to register the branch and obtain an `analysisUuid`, which is stamped into `metadata.analysis_uuid` (proto field 19) so the CE binds the report to the branch (without it the CE accepts the report but creates no branch). If the main branch CE task fails, remaining branches are skipped; a branch whose source is no longer retrievable on the server is also skipped with a clear message. Supports `ExcludeBranches` glob patterns to skip non-main branches, and per-branch checkpoint/resume via `loadCompletedBranches()`/`shouldSkipBranch()` — that pair resumes only *within* a run, since its JSONL lives in the run directory. A second, independent filter stage, `filterBranchesByAnalyzedAfter`, selects branches by last-analysis date from `--branch_analyzed_after`; the project's main branch is always force-included if the date filter would otherwise exclude every branch (#583). Extract applies the same date gate inside `buildBranchMap` to skip per-branch extraction entirely for excluded branches. Across runs, `targetBranchUpToDate()` skips any branch whose analysis date on the target is already at or after the date this run would submit (#588): reports are backdated to the source's real last-analysis date, so a re-run would otherwise resubmit that identical date and the CE rejects it ("a newer report has already been processed, and processing older reports is not supported"). The dates come from the `Branches.List` call the task already makes for the main-branch name, so the check costs no extra request; such branches are recorded as `up_to_date`, which `collectProjectData` counts as a success. **Concurrency (#596).** Project-level concurrency uses `errgroup.WithContext` plus a `DynamicGate`, and branch-level concurrency a second, run-wide gate (`Executor.BranchGate`). Both are sized off `Executor.CEPollConcurrencyLimiter` rather than the latency-driven `Executor.ConcurrencyLimiter` introduced in #573: a slot here is held across `buildBranchReport` → `/api/ce/submit` → `PollCETask`, so what bounds it is the rate at which those slots hit `api/ce/task`, not raw HTTP call latency. `pollBoundConcurrency()` derives that figure statically as `apiMaxRatePerMin × scanreport.AveragePollInterval(typicalCETaskDuration) / 60` — 25 slots at the top of the `--api_max_rate_per_min` range. `BranchGate` is deliberately **one gate for the whole run**, not one per project: a per-project gate would make in-flight work the *product* of the two levels (25 projects × up to `MaxBranchesPerProject` branches), roughly ten times the budget. It is also a separate `DynamicGate` instance from the project gate, because a single gate shared across a nested fan-out deadlocks — a project holding the only slot would wait forever for a branch slot. Branch slots are safe to wait on because nothing holding one ever needs a project slot. **CE polling (#596).** `PollCETask` no longer polls on a fixed 10s cadence; it polls immediately, then backs off `CEPollInitialInterval` (1s) × `CEPollBackoffFactor` (2) up to `CEPollMaxInterval` (10s). Measured over 603 real CE tasks, median execution is 1.57s and p90 2.95s, so most tasks now return on the first or second poll instead of waiting out a full fixed interval. The faster early rungs are exactly why the slot count above had to shrink: a shorter average poll interval spends more of the `api/ce/task` budget per slot.
...
covering retries / branch skips / forced main branches (#583) / gate-condition skips / metric remaps, per-branch stats, and
